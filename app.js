...
function initials(name) {
  return name.split(" ").filter(Boolean).slice(-2).map((p) => p[0]).join("").replace(".", "").slice(0, 2).toUpperCase();
}
...
function payslip(emp) {
  ...
  const net = gross - nasfund - tax - loan;
  return { gross, nasfund, employerNasfund, tax, loan, net, loans };
}
...
function route() {
  ...
  const hr = {
    ...
    leave: renderLeaveHr, jobs: renderJobsHr, loans: renderLoansHr, payroll: renderPayrollHr
  };
  const staff = {
...
function renderEmployees() {
  ...
  return `
    ...
    <div class="cards">${list.map((e) => `
      <article class="person clickable" data-emp="${e.id}" role="button" tabindex="0"><div class="row-between"><div class="person-top"><div class="avatar">${initials(e.name)}</div>
...
function renderPayrollHr() {
  ...
  return `
    ...
    <div class="card" style="overflow:auto">
      <table class="plain"><thead><tr><th>Staff</th><th>Gross</th><th>NASFUND</th><th>PAYE</th><th>Loan</th><th>Net</th></tr></thead>
      <tbody>${rows.map((r) => `<tr><td>${r.e.name}<div class="meta">${r.e.dept}</div></td><td>${kina2(r.p.gross)}</td><td>${kina2(r.p.nasfund)}</td><td>${kina2(r.p.tax)}</td><td>${kina2(r.p.loan)}</td><td><strong>${kina2(r.p.net)}</strong></td></tr>`).join("")}</tbody></table>
    </div>`;
}
...
function renderPayslip() {
  ...
  return `
    ...
    <div class="grid-2">
      <div class="card">
        <h3>This month</h3>
        <div class="pay-row"><span>Gross (annual ÷ 12)</span><span>${kina2(p.gross)}</span></div>
...
function renderProfile() {
  ...
  return `
    ...
    <div class="card clickable" style="max-width:560px" data-emp="${person.id}" role="button" tabindex="0">
      <div class="person-top"><div class="avatar">${initials(person.name)}</div>
  ...
  showModal(`
    ...
    ${staff.length ? staff.map((e) => `
      <button type="button" class="app-row" data-emp="${e.id}" data-from-dept="${d.id}">
        <div class="person-top"><div class="avatar">${initials(e.name)}</div>
    ...
    ? (apps.length ? apps.map((a) => {
        ...
        return `
          <button type="button" class="app-row" data-app="${a.id}">
            <div class="person-top"><div class="avatar">${initials(a.employee)}</div>
...
function openModal(kind) {
  ...
  const forms = {
    employee: `<h3>Add employee</h3><form class="form-grid" id="f-emp">
      ...
      <p class="meta span-2">They can sign in on Staff self-service with this email and password unre2026.</p>
...
document.getElementById("modal").addEventListener("submit", async (e) => {
  ...
  if (form.id === "f-emp") {
    state.employees.push({ id: uid("e"), name: data.name, role: data.role, type: data.type, dept: data.dept, email: data.email.toLowerCase(), status: "Active", salary: Number(data.salary), password: DEMO_PASS });
